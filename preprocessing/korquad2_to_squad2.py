...
from .korquad2_preprocess import get_wiki_context
...
def build_parser():
    ...
    # BASIC OPTION
    parser.add_argument("--worker", dest="worker", default=20,
                        type=int, help='multiprocess 갯수')
    parser.add_argument("--data_path", dest="data_path",
                        default='data/korquad/dev', type=str, help='전환하려는 데이터가 들어있는 폴더')
    parser.add_argument("--save_path", dest="save_path",
                        default='data/korquad/converted_dev', type=str, help='전환후 데이터가 저장될 폴더')
    parser.add_argument("--merge_name", dest="merge_name",
                        default='convert_sample_v2.json', type=str, help='병합파일의 이름')

    # CONVERT OPTION
    parser.add_argument("--max_paragraph_length", dest="max_paragraph_length",
                        default=100000000, type=int, help='단락의 길이')  # 사용안함 .....
    parser.add_argument("--max_answer_text_length", dest="max_answer_text_length",
                        default=100000000, type=int, help='질문의 길이')  # 사용안함 .....
...
def convert_file_into_squad(config):
    ...
    if config.worker > 1:
        ...
        file_paths = [os.path.join(config.data_path, file_name)
...
class Korquad2_Converter(object):
    def __init__(self, max_paragraph_length=1000000, max_answer_text_length=1000000):
        self.parser = {
            'table': ['{', '}'],
            'ul': ['{', '}'],
            'dl': ['{', '}'],
    ...
    # def convert_to_squad_format(self, data):
    ...
    def convert_to_squad_format(self, html, qas):
        ...
        # 문장 길이 단위
        # context_list, context_pos_list = self.merge_structure_contexts(
        #     structure_contexts)
        # paragraphs, paragraph_ids = self.merge_contexts_by_len(
        #     context_list, context_pos_list, self.max_paragraph_length)
        # 문단 단위
        paragraphs, paragraph_ids = self.merge_structure_contexts(
            structure_contexts)

        modified_paragraphs = []
        for idx, (paragraph, paragraph_id) in enumerate(zip(paragraphs, paragraph_ids)):
            modified_paragraph = self.get_modified_paragraph(
                paragraph, paragraph_id, qas)
            if modified_paragraph is not None:
                modified_paragraphs.append(modified_paragraph)
        
        # 정답 있는 것만 필터링
        # modified_results = []
        # for result in modified_paragraphs:
        #     modified_qas = list(filter(lambda x: not x['is_impossible'], result['qas']))
        #     if modified_qas == []:
        #         continue
        #     modified_result = {'context': result['context'], 'qas': modified_qas}
    ...
    def get_modified_paragraph(self, paragraph, paragraph_id, qas):
        ...
        for qa in qas:
            ...
            unique_id = qa['id']
            modified_qa = {"question": question, 'answers': [],
                           'is_impossible': True, 'id': unique_id}
            answer_start = qa['answer']['answer_start']
            text = qa['answer']['text']
            answer_end = answer_start + len(text) - 1
            possible_ids = np.where((paragraph_id >= answer_start) & (
                paragraph_id <= answer_end))[0]
            if len(possible_ids) > 0 and possible_ids is not None:
                ...
                if modified_text is not None and len(modified_text) > 0 and not modified_text == '':
                    ...
                    modified_answer = {'text': modified_text,
                                       'answer_start': int(modified_start)}
        ...
        if len(modified_qas) > 0:
            ...
            #modified_paragraph = {'context': modified_paragraph, 'qas': modified_qas}
            return modified_paragraph
        return None

    def merge_structure_contexts(self, structure_contexts):
        # h2, h3로 문단이 나누므로 = 토큰을 사용하여 분리
        h_token = " == "
        h_idx = -1
        ...
        for temp_context in structure_contexts:
            ...
            if previous_tag in ['h2', 'h3']:
                context, context_pos, previous_tag = self.merge_structure_text(
                    temp_context, previous_tag, [], [])
    ...
    def merge_structure_text(self, structure_text, previous_tag, context, context_pos):
        ...
        if type(text) == list and tag_name in ['table', 'ul', 'dl', 'tr']:
            ...
            if tag_name in ['td', 'th'] and previous_tag in ['td', 'th']:
                ...
                context_pos[-1] = join_list([context_pos[-1],
                                             text_pos], self.connector[tag_name])
        ...
        for context, context_id in zip(contexts, context_ids):
            if stack_context is None and stack_context_id is None:
                ...
                if len(stack_context_id) + len(context_id) < max_paragraph_length:
                    ...
                    stack_context_id = join_list(
                        [stack_context_id, context_id], sep_token)
                else:
